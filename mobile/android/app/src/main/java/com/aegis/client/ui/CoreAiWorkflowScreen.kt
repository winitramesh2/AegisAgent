    ...
    LazyColumn(
        ...
        item {
            IncidentInputCard(
                ...
                placeholder = "Example: OTP code not generating on Android",
                helperText = "Cloud mode gives the deepest analysis and verifies with local model signals.",
                primaryLabel = "Diagnose",
                primaryIcon = Icons.AutoMirrored.Filled.Send,
                onPrimaryClick = onDiagnose,
                primaryEnabled = state.query.isNotBlank() && !state.isDiagnosing && !state.isRetrying && !state.isEscalating
            )
        }
        ...
        item {
            ModeInfoCard(
                ...
                description = "Your issue is analyzed in cloud first, then cross-verified locally. Use this mode for the highest-quality troubleshooting."
            )
        }

        if (state.isDiagnosing || state.isRetrying || state.isEscalating) {
            item {
                Card {
                    Row(
                        ...
                        Text(
                            when {
                                state.isDiagnosing -> "Analyzing with cloud and local verification..."
                                state.isRetrying -> "Running retry diagnosis..."
                                else -> "Creating escalation ticket..."
                            }
                        )
                    }
                }
        ...
        if (state.actionChecklist.isNotEmpty() && state.stage != WorkflowStage.Resolved && state.stage != WorkflowStage.Escalated) {
            item {
                ActionChecklistCard(actions = state.actionChecklist, onToggleAction = onToggleAction)
            }
        }
        ...
        if (state.stage == WorkflowStage.Resolved) {
            item {
                InlineStatusMessage(
                    message = "Resolved. Great! You can edit the issue and diagnose again anytime.",
                    isError = false
                )
            }
        }
            ...
            if (chat != null) {
                item {
                    EscalationStatusCard(
                        ticketId = chat.escalationTicketId,
                        message = chat.message
                    )
                }
            }
